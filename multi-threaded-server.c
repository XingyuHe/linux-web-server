 ...
 */
static int handleFileRequest(const char *requestURI, int clntSock)
{
    int statusCode;
    FILE *fp = NULL;
...
void *worker(void *clntSockPtr)
{
    ...
    if (getpeername(clntSock, (struct sockaddr *) &clntAddr, (unsigned int * restrict) &clntSize) < 0)
    ...
    extraThingsOnRequestLine = strtok_r(NULL, token_separators, &saveptr);
    ...
    statusCode = handleFileRequest(requestURI, clntSock);
...
loop_end:
    ...
    inet_ntop(AF_INET, &clntAddr.sin_addr, line, sizeof(clntAddr.sin_addr));
...
int main(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        // create thread 
        pthread_t thread_id;
        pthread_create(&thread_id, NULL, worker, (void *) &clntSock);
